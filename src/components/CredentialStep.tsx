...
import { credentialProviders, credentialStates } from "@/lib/api/verify.functions";

type CredentialState = { state_name: string; state_code: string };
...
// vc `required_fields` aliases → the param names Verify's run/credential expects.
...
/**
 * License step for the demo: pick state → license type → fill required fields,
 * then hand the payload back to the flow to run the credential check. In KYC mode
 * (`kycName` set) the name comes from the verified ID, so name inputs are hidden.
 */
export function CredentialStep({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const r = await credentialStates({ data: { token } });
      if (!alive) return;
  ...
  const onStateChange = async (s: string) => {
    ...
    const r = await credentialProviders({ data: { token, state: s } });
  ...
  const submit = () => {
    ...
    if (missing.length) return setErr(`Please fill: ${missing.map(label).join(", ")}.`);
        ...
        {fields.map((f) => (
          <div key={f}>
            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wide text-muted-foreground">
              {label(f)}
              {f === "npi" && (
                <span className="ml-1 normal-case text-muted-foreground/70">(optional)</span>
              )}
            </label>
            <input
              className={inputClass}
