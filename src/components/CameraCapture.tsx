...
/**
 * Live camera capture with a canvas snapshot, a review/retake step, and a
 * file-upload fallback (for desktops without a camera or denied permission).
...
export function CameraCapture({ facingMode, overlay, title, hint, onCapture, onSkip }: Props) {
  ...
  const capture = () => {
    ...
    if (!ctx) return;
    // Mirror selfies so the preview matches what the user expects.
    if (facingMode === "user") {
  ...
  const frameClass =
    overlay === "oval"
      ? "aspect-[3/4] [clip-path:ellipse(42%_46%_at_50%_50%)]"
      ...
      <div className="relative mx-auto max-w-sm overflow-hidden rounded-2xl border border-border bg-foreground/5">
        ...
        {!shot && !error && (
          <div aria-hidden className="pointer-events-none absolute inset-0 flex items-center justify-center p-6">
            <div className={`w-full border-2 border-white/80 shadow-[0_0_0_9999px_rgba(0,0,0,0.35)] ${frameClass}`} />
          </div>
        )}
      ...
      <div className="mt-5 flex flex-col items-center gap-3">
        ...
        {!shot && (
          <label className="inline-flex items-center gap-2 text-sm text-muted-foreground cursor-pointer hover:text-foreground transition-colors">
            ...
            <input type="file" accept="image/*" capture={facingMode === "user" ? "user" : "environment"} className="hidden" onChange={onFile} />
          </label>
        )}
